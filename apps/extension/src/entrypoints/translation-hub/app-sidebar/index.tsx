    ...
    <Sidebar collapsible="icon">
      <SidebarHeader className="group-data-[state=expanded]:px-5 group-data-[state=expanded]:pt-4 transition-all">
        <a href="https://readfrog.app" className="flex items-center gap-2">
          ...
          <span className="text-lg font-bold overflow-hidden truncate">{i18n.t('name')}</span>
      ...
      <SidebarContent className="group-data-[state=expanded]:px-2 transition-all">
        <SidebarGroup>
          <SidebarGroupLabel>{i18n.t('translationHub.services.title')}</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
              {Object.entries(NAV_ITEMS).map(([key, item]) => (
                <SidebarMenuItem key={key}>
                  <SidebarMenuButton asChild isActive={location.pathname === item.url}>
                    <Link to={item.url}>
                      <Icon icon={item.icon} />
                      <span>{i18n.t('translationHub.header.title')}</span>
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
              ))}
